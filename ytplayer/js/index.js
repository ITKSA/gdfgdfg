...
$(function() {
    ...
    $('#queue-add-song-button').on('click', () => {
        helpers.getSongModal('اضافة اغنيه الى قائمة الانتظار', 'اسم الاغنيه او الرابط', 'اضافة', 'https://youtu.be/فديتك يالعذب الحفوق', () => {
    ...
    $('#playlist-add-song-button').on('click', () => {
        helpers.getSongModal('اضافة اغنيه الى البلي لست', 'اسم الأغنيه او رابط', 'اضافه', 'https://youtu.be/تخيل انه هنا فيه رابط شي عجيب صح ؟', () => {
            let song = $('#song-url').val();
    ...
    $('#load-playlist-button').on('click', () => {
        player.dbQuery('get_playlists', 'yt_playlists_registry', (results) => {
            ...
            helpers.getPlaylistModal('اضافة بلي لست موجود', 'الأسم', 'حفظ', 'بلي لست', playlists, () => {
                let playlist = $('#playlist-load').find(':selected').text();
